...
hpet_enable_interrupts_tim1(void) {
    ...
    // enable timer 1 to send interrupts
    hpetReg->TIM0_CONF |= HPET_TN_INT_ENB_CNF;
    // enable timer 1 to send periodic interrupts
    hpetReg->TIM0_CONF |= HPET_TN_TYPE_CNF;
    // for periodic mode
    hpetReg->TIM0_CONF |= HPET_TN_VAL_SET_CNF;
    // when the next interrupt will happen
...
hpet_cpu_frequency(void) {
    ...
    if (!cpu_freq) {
        ...
        const uint64_t time_int = 100;
        //hpetFreq / 100 -> Hz per 0.01 second 
        const uint64_t target = hpetFreq / time_int;
        //initial counter value
        uint64_t counter = 0;
        uint64_t start = hpet_get_main_cnt();
        //get clocks before execution
        uint64_t tsc_before = read_tsc();
        do {
        ...
        //get clocks after execution
        uint64_t tsc_after = read_tsc();
        //tsc_after - tsc_before clocks execute in 0.01 second 
        //multiply them by time_interval to get clocks per second
...
pmtimer_cpu_frequency(void) {
    ...
    if (!cpu_freq) {
        //choose time interval 
        const uint64_t time_int = 100;
        //PM_FREQ / 100 -> Hz per 0.01 second 
        const uint64_t target = PM_FREQ / time_int;
        //initial counter value
        uint32_t counter = 0;
