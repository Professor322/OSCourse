...
sys_map_region(envid_t srcenvid, uintptr_t srcva,
    ...
    if (perm & ~PROT_ALL || perm & ALLOC_ONE || perm & ALLOC_ZERO) 
        return -E_INVAL;
    //user_mem_assert(src, (void*)srcva, size, perm | PROT_USER_);
    //user_mem_assert(dst, (void*)dstva, size, perm | PROT_USER_);
...
sys_ipc_try_send(envid_t envid, uint32_t value, uintptr_t srcva, size_t size, int perm) {
    ...
    if (srcva < MAX_USER_ADDRESS && env->env_ipc_dstva < MAX_USER_ADDRESS) {
        ...
        res = map_region(&env->address_space, env->env_ipc_dstva, 
                         ...
                         env->env_ipc_maxsz, perm | PROT_USER_);
        if (res < 0)
            return res;
...
//maps 2 displays to UTEMP and UTEMP + context.size
static int
