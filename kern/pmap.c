...
dump_memory_lists(void) {
    ...
    for (int i = 0; i < MAX_CLASS; ++i) {
        struct List *begin = &free_classes[i];
        if (!list_empty(begin)) {
            for (struct List *temp = begin->next; temp != begin; temp = temp->next) {
                struct Page *page = (struct Page*)temp;
...
#define LAST_ALLOC_SIZE 5
struct Page *last_alloc[LAST_ALLOC_SIZE];
size_t last_alloc_actual_size = 0;
...
map_page(struct AddressSpace *spc, uintptr_t addr, struct Page *page, int flags) {
    ...
    size_t pdi0 = PD_INDEX(addr), pdi1 = PD_INDEX(end);
    if (pdi0 > pdi1) pdi1 = PD_ENTRY_COUNT;
    //size of page class == 9 -> 2048 bytes which is the size of the entry in pdp
