  ...
  )
{
  ...
  //
  /*DEBUG((DEBUG_INFO, "JOS: Trying to set graphics card mode to max resolution...\n"));
  for (UINT32 i = 0; i < GraphicsOutput->Mode->MaxMode; ++i) {
    ...
    if (EFI_ERROR (Status)) {
        DEBUG ((DEBUG_ERROR, "JOS: Cannot query info about mode: %d - %r\n", i, Status));
        return Status;
    }
    if (Info->HorizontalResolution == 1024 &&
        ...
        if (EFI_ERROR (Status)) {
            DEBUG ((DEBUG_ERROR, "JOS: Cannot change graphics mode - %r\n", Status));
            return Status;
        }
     }  
  }*/
  Status = GraphicsOutput->SetMode(GraphicsOutput,15);
  DEBUG((DEBUG_INFO, "JOS: Highest resolution mode is: %d\n", GraphicsOutput->Mode->Mode));
